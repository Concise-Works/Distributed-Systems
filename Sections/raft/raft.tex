...
\begin{Def}[Raft - Linearizing Client Interactions w/ IDs]
    ...
    \noindent
    Clients first connect to a random server, and are redirected if not the leader server.
    Server crashes timeout clients, for which clients retry their requests with another server. Raft 
